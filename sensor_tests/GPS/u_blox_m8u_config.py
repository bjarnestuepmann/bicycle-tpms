...
def update_status_esf_status(msg: UBXMessage):
    ...
    # Calibration Status
    # 6 Sensors: 3-axis ACC, 3x-axis GYRO
    # each sensor has own calibration status
    calibStatusList = []
    ...
    if min(calibStatusList) == 0:
        print("Calib Status: INITIALIZING")
        CALIB_STAUTS = CalibStatus.INITIALIZING
    elif min(calibStatusList) == 1:
        print("Calib Status: CALIBRATING")
        CALIB_STAUTS = CalibStatus.CALIBRATING
    else:
        print("Calib Status: CALIBRATED")
        ...
        while not stop_event.is_set():
            try:
                if(CALIB_STAUTS == CalibStatus.INITIALIZING):
                    ...
                    while(CALIB_STAUTS == CalibStatus.INITIALIZING):
                        sleep(SLEEP_TIME)
                    print("Done\n")
